...
from human_pose_util.dataset.group import copy_group, filter_children, mapped_group, KeySubsetGroup
...
def apply_space_scale(sequence,
                      space_scale,
                      div_keys=['p3w', 'p3c'],
                      div_attr_keys=['t']):
...
def apply_pixel_scale(sequence,
                      pixel_scale,
                      div_keys=['p2'],
                      div_attr_keys=['f', 'c']):
    for k in div_keys:
...
def apply_skeleton_conversion(dataset,
                              skeleton_id,
                              pose_keys=['p3c', 'p3w', 'p2']):
...
def _get_heights(sequences, skeleton):
    ...
    heights = {
        k: np.max(skeleton.height(np.concatenate(v, axis=0)))
...
def filter_dataset(dataset,
    ...
    if len(conds) > 0:
        dataset = filter_children(dataset,
                                  lambda k, v: all([cond(v) for cond in conds]))
    if keys is not None:
...
def dataset_to_p3w(dataset, rotate_front=False, recenter_xy=False):
...
def dataset_to_view_data(dataset):
    ...
    r, t, f, c = [
        np.concatenate([[s.attrs[k]
                         for i in range(s.attrs['n_frames'])]
...
def normalized_p3w(dataset,
    ...
    skeleton_id = dataset.attrs['skeleton_id']
    return dataset, dataset_to_p3w(dataset,
                                   rotate_front=rotate_front,
                                   recenter_xy=recenter_xy)


def normalized_view_data(dataset,
                         modes=None,
                         camera_idxs=None,
                         keys=None,
                         skeleton_id=None,
