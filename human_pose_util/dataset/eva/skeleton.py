...
class _S14(Skeleton):
    ...
    def height(self, p3):
        return skeleton_height(p3,
                               l_foot_index=self.joint_index(l_ankle),
...
class _S20(Skeleton):
    ...
    def from_raw(self, pose):
        ...
        return np.stack([
            np.array([i[0][0, 0] for i in pose[k]]) for k in self._raw_indices
        ],
                        axis=0)
...
class _S20ToS16Converter(SkeletonConverter):
    ...
    def _calc_common_indices(self):
        """Used in `to_s16`."""
        common = ((head, head_distal), (neck, head_proximal), (thorax,
                                                               torso_proximal),
                  (pelvis, torso_distal), (l_shoulder, l_upper_arm_proximal),
                  (l_wrist, l_lower_arm_distal), (r_shoulder,
                                                  r_upper_arm_proximal),
                  (r_wrist, r_lower_arm_distal), (l_hip, l_upper_leg_proximal),
                  (l_ankle, l_lower_leg_distal), (r_hip, r_upper_leg_proximal),
    ...
    def convert(self, data):
        ...
        if shape[-2] != 20:
            raise Exception('Invalid number of joints for native: %d' %
                            shape[-2])
        shape[-2] = 16
