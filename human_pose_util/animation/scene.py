...
from human_pose_util.animation.grid import grid
from human_pose_util.animation.skeleton_vis import LimbCollection
...
@window.event  # pylint: disable=no-member
def on_draw(dt):
    window.clear()  # pylint: disable=no-member
    ticks.draw()
...
for s in ['transform', 'viewport']:
    window.attach(ticks[s])  # pylint: disable=no-member
