...
def convert():
    ...
    def _p2_path(subject, sequence, view):
        return os.path.join(dataset_dir, 'D2_positions', subject,
                            '%s.%s.cdf' % (sequence, view))

    def _p3_path(subject, sequence, view=None):
        if view is None:
            return os.path.join(dataset_dir, 'D3_positions', subject,
                                '%s.cdf' % sequence)
        else:
            return os.path.join(dataset_dir, 'D3_positions_mono', subject,
                                '%s.%s.cdf' % (sequence, view))

    def _video_path(subject, sequence, view):
        return os.path.join(dataset_dir, 'Videos', subject,
                            '%s.%s.mp4' % (sequence, view))
    ...
    def _views(subject, sequence):
        ...
        return (p[n + 1:-4]
                for p in paths
                if len(p) > n + 1 and p[:n + 1] == '%s.' % sequence)
...
class RootNode(object):
    ...
    def subjects(self,
        ...
        subjects = _subject_codes(include_train=include_train,
...
class ViewNode(object):
    ...
    def video_path_short(self):
        ...
        s = self._group.name.split('/')
        subject_string = s[2]
        sequence_string = s[4]
        view_string = s[6]
        return os.path.join(subject_string,
                            '%s.%s.mp4' % (sequence_string, view_string))
