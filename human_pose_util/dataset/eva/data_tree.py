...
from human_pose_util.dataset.eva.raw_tree.path import video_path
from human_pose_util.dataset.eva.meta import partitions, n_frames
...
class SubjectNode(TreeNode):
    ...
    def sequences(self, base_code=None):
        return [
            self.sequence(c) for c in self.sequence_codes(base_code=base_code)
        ]
...
class SequenceNode(TreeNode):
    ...
    @property
    def n_frames(self):
        return n_frames[self.subject_code][self.base_sequence_code][
            self.trial_index]
...
class ViewNode(TreeNode):
    ...
    @property
    def video_path(self):
        return video_path(self.subject_code, self.sequence_code,
                          self.camera_code)
