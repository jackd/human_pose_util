...
class SkeletonConverter(object):
    ...
    def __init__(self, input_skeleton, output_skeleton, oi_map={}):
        self._indices = tuple(
            get_transform_indices(input_skeleton, output_skeleton, oi_map))
...
class ExpandingSkeletonConverter(SkeletonConverter):
    ...
    def __init__(self,
                 ...
                 empty_value=0):
        self._indices = tuple(
            get_transform_indices(output_skeleton, input_skeleton,
...
class ExpandingMidJointSkeletonConverter(ExpandingSkeletonConverter):
    ...
    def __init__(self,
                 ...
                 oi_map={},
                 empty_value=None):
        super(ExpandingMidJointSkeletonConverter,
              self).__init__(input_skeleton,
                             output_skeleton,
                             oi_map,
                             empty_value=empty_value)
        if isinstance(mid_ids, six.string_types):
            if not isinstance(left_ids, six.string_types):
                raise ValueError('left_ids must be a str if mid_ids is')
            if not isinstance(right_ids, six.string_types):
    ...
    def convert(self, input_data, axis=-2):
        out = super(ExpandingMidJointSkeletonConverter,
                    self).convert(input_data, axis=axis)
...
class CompoundConverter(SkeletonConverter):
    ...
    def __init__(self, *converters):
        ...
        for i in range(len(converters) - 1):
            if converters[i].output_skeleton != converters[i +
                                                           1].input_skeleton:
