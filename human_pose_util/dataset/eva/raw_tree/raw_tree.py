...
from human_pose_util.dataset.eva.raw_tree.path import mocap_data_path, cal_path
from human_pose_util.dataset.eva.raw_tree.camera_parameter import CameraParameter
from human_pose_util.dataset.eva.raw_tree.actor_parameter import ActorParameter
from human_pose_util.dataset.eva.raw_tree.motion_capture_data import MotionCaptureData
from human_pose_util.dataset.eva.raw_tree.conic_limb_parameter import ConicLimbParameter
from human_pose_util.dataset.eva.raw_tree.limb_length import LimbLength
from human_pose_util.dataset.eva.raw_tree.global_marker_transform import GlobalMarkerTransform
from human_pose_util.dataset.eva.raw_tree.synchronization_parameter import SynchronizationParameter
from human_pose_util.dataset.eva.raw_tree.pose_2d import Pose2D
from human_pose_util.dataset.eva.raw_tree.pose_3d import Pose3D
from human_pose_util.dataset.eva.data_tree import RootNode, SubjectNode, SequenceNode, ViewNode
from human_pose_util.dataset.eva.raw_tree.path import mp_path, ofs_path
...
class _SequenceNode(SequenceNode):
    ...
    def conic_limb_parameters(self):
        return ConicLimbParameter(self.mocap_data, self.subject.actor_params)
    ...
    def global_marker_transform(self, mocap_frame):
        return GlobalMarkerTransform(self.mocap_data, mocap_frame,
                                     self.conic_limb_parameters,
                                     self.limb_length(mocap_frame))

    def p3_frame(self, mocap_frame):
        return Pose3D(self.global_marker_transform(mocap_frame),
                      self.limb_length(mocap_frame))
...
class _ViewNode(ViewNode):
    ...
    def mocap_frame(self, image_frame):
        if self.sequence.has_video:
            ...
            return sync_param.mc_st + (image_frame -
                                       sync_param.im_st) * sync_param.mc_sc
        else:
            return image_frame

    def pose2d(self, image_frame):
        mocap_frame = self.mocap_frame(image_frame)
        return Pose2D(self.sequence.p3_frame(mocap_frame), self.camera_params)
    ...
    def video_n_frames(self):
        return int(self._get_from_video(cv2.CAP_PROP_FRAME_COUNT))  # pylint: disable=no-member

    @cached_property
    def video_fps(self):
        return self._get_from_video(cv2.CAP_PROP_FPS)  # pylint: disable=no-member

    def _get_from_video(self, code):
        cap = cv2.VideoCapture(self.video_path)  # pylint: disable=no-member
        val = cap.get(code)
        cap.release()
        return val
    ...
    def camera_extrinsics(self):
        ...
        r = np.array(euler_from_matrix_nh(camera_params.Rc))
