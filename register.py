...
def register_skeletons(h3m=False, eva=False, mpi_inf=False):
    ...
    if eva:
        ...
        from dataset.eva.skeleton import s20_to_s14_converter
        from dataset.eva.skeleton import s20_to_s16_converter
        for k, s in [('s14', s14), ('s16', s16), ('s20', s20)]:
            register_skeleton(k, s)
        for k0, k1, converter in [
                ['s20', 's14', s20_to_s14_converter()],
                ['s20', 's16', s20_to_s16_converter()]]:
            register_converter(k0, k1, converter)
    if mpi_inf:
        from dataset.mpi_inf.skeleton import relevant, base, extended
        for k, s in [
                ('mpi-inf-base', base),
                ('mpi-inf-relevant', relevant),
...
def register_datasets(h3m=False, eva=False, mpi_inf=False):
    ...
    if h3m:
        from dataset.h3m.dataset import H3mDataset
        register_dataset('h3m', H3mDataset())
    if eva:
        from dataset.eva.dataset import EvaDataset
